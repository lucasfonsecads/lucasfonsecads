...
dag = DAG('get_data_book_6',
          description='Dag TJSP - Book Six',
...
def upload_file(file_name, bucket, week, object_name):
    ...
    new_object_name = 'week-{0}-{1}-book-6/{2}'.format(week, state, object_name)
...
def get_book():
    ...
    try:
        ...
        for control in range(len(week_period)):
            ...
            response = requests.get('{0}?dtDiario={1}&cdCaderno=15'.format(URL, week_period[control]))
