...
dag = DAG('get_data_book_2',
          description='Dag TJSP - Book Two',
...
def upload_file(file_name, bucket, week, object_name):
    ...
    new_object_name = 'week-{0}-{1}-book-2/{2}'.format(week, state, object_name)
...
def get_book():
    ...
    try:
        ...
        for control in range(len(week_period)):
            ...
            response = requests.get('{0}?dtDiario={1}&cdCaderno=11'.format(URL, week_period[control]))
